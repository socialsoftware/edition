	...
	<div class="container">
		<div class="col-md-8 col-md-offset-2 ldod-about">
			<h1 class="text-center">
				<spring:message code="header.encoding" />
			</h1>
			<p>&nbsp;</p>
			<c:choose>
				<c:when
					test='${pageContext.response.locale.getLanguage().equals("en")}'>
					<%@ include file="/WEB-INF/jsp/about/encoding-en.jsp"%>
				</c:when>
				<c:when
					test='${pageContext.response.locale.getLanguage().equals("es")}'>
					<%@ include file="/WEB-INF/jsp/about/encoding-es.jsp"%>
				</c:when>
				<c:otherwise><%@ include file="/WEB-INF/jsp/about/encoding-pt.jsp"%></c:otherwise>
			</c:choose>
		</div>
