...
public class LoadTEIFragments {
        ...
        for (String xmlId : listXmlId) {
            ...
            if (objects2 == null) {
                throw new LdoDLoadException(
                        "identificador não declarado " + xmlId);
            }
        ...
        for (Content content : element.getContent()) {
            if (content.getCType() == CType.Text) {
                ...
                if (element2.getName().equals("div")) {
                    ...
                    throw new LdoDLoadException(
                            "DOES NOT HANDLE LOAD OF:" + element2 + " OF TYPE:"
        ...
        for (Content content : element.getContent()) {
            if (content.getCType() == CType.Text) {
                ...
                if (element2.getName().equals("ref")) {
                    ...
                    throw new LdoDLoadException(
                            "não carrega elementos: " + element2 + " do tipo:"
                                    + element2.getName() + " dentro de note "
                                    + " _VALOR=" + element.getText());
                }
            }
        }
    }
    ...
    private void loadAlt(Element element, TextPortion parent) {
        ...
        for (String xmlId : targetList) {
            List<Object> listSegTextList = getObjectDirectIdMap(
                    xmlId.substring(1));
    ...
    private void loadUnclear(Element element, TextPortion parent) {
        ...
        if (contentList.size() != 1)
            throw new LdoDLoadException(
                    "unclear não contém apenas texto" + element.getText());
        ...
        for (Content content : element.getContent()) {
            if (content.getCType() == CType.Text) {
                ...
                if (element2.getName().equals("add")) {
                    ...
                    throw new LdoDLoadException(
                            "não carrega elementos: " + element2 + " do tipo:"
                                    + element2.getName() + " dentro de subst"
                                    + " _VALOR=" + element.getText());
                }
            }
        }
        ...
        for (Content content : element.getContent()) {
            if (content.getCType() == CType.Text) {
                ...
                if (element2.getName().equals("add")) {
                    ...
                    throw new LdoDLoadException(
                            "não carrega elementos: " + element2 + " do tipo:"
                                    + element2.getName() + " dentro de del "
                                    + " _VALOR=" + element.getText());
                }
            }
        }
    ...
    private void loadSeg(Element element, TextPortion parent) {
        ...
        if (contentList.size() != 1)
            throw new LdoDLoadException(
                    "seg não contém apenas texto" + element.getText());
        ...
        if (edAttribute == null) {
            ...
            for (FragInter inter : toFragInters) {
                if (!parent.getInterps().contains(inter)) {
                    throw new LdoDLoadException("testemunho com identificador:"
                            + inter.getXmlId()
    ...
    private void loadFacsimile(String fragmentTEIID) {
        ...
        for (Element facsElement : xp.evaluate(doc)) {
            ...
            if (correspAtt != null) {
                ...
                throw new LdoDLoadException("elemento facsimile xml:id=" + xmlID
                        + " necessita de atributo corresp");
            }
    ...
    private void loadSurface(Element facsElement, Facsimile facsimile) {
        for (Element surfElement : facsElement.getChildren("surface",
                namespace)) {
    ...
    private void loadWitnesses(Fragment fragment, String fragmentTEIID) {
        ...
        for (Element witness : xp.evaluate(doc)) {
            ...
            assert (objects != null) && (!objects
                    .isEmpty()) : "MISSING SOURCE OBJECT FOR xml:id:"
                            + sourceOrEditionXmlID;
            ...
            if (witnessXmlID == null)
                throw new LdoDLoadException(
                        "elemento wit sem atributo xml:id=" + witnessXmlID);
            ...
            if (bibl != null) {
                ...
                if (dateElement != null) {
                    ...
                    if (whenAttribute == null) {
                        ...
                        fragInter.setDate(DateUtils
                                .convertDate(whenAttribute.getValue()));

                        PrecisionType precision = getPrecisionAttribute(
                                dateElement);
                        if (precision != null) {
    ...
    private void setNotes(FragInter fragInter, Element bibl) {
        ...
        for (Element noteElement : notesList) {
            ...
            if (typeValue.equals("physDesc")) {
                    ...
                    if (content.getCType() == CType.Text) {
                        ...
                        if (element2.getName().equals("ref")) {
                            ...
                            if (refType == RefType.GRAPHIC) {
                                ...
                                Fragment frag = LdoD.getInstance()
                                        .getFragment(target);
                                // if (frag != null) {
                ...
                throw new LdoDLoadException(
                        "atributo type de elemento note com valor="
                                + typeValue);
            }
        ...
        for (Element biblScope : bibl.getChildren("biblScope", namespace)) {
            ...
            case "pp":
                if (fragInter instanceof ExpertEditionInter) {
                    if (value != "") {
                        ((ExpertEditionInter) fragInter)
                                .setStartPage(Integer.parseInt(value));
                        ((ExpertEditionInter) fragInter)
                                .setEndPage(Integer.parseInt(value));
                    } else {
                        String from = biblScope.getAttributeValue("from");
                        String to = biblScope.getAttributeValue("to");

                        ((ExpertEditionInter) fragInter)
                                .setStartPage(Integer.parseInt(from));
                        ((ExpertEditionInter) fragInter)
                                .setEndPage(Integer.parseInt(to));
                    }
                }
                break;
            case "vol":
    ...
    private Heteronym getHeteronym(Element bibl) {
        ...
        if (respStmt != null) {
            ...
            if (persName != null) {
                ...
                if (correspAtt != null) {
                    hetXmlId = persName.getAttributeValue("corresp")
                            .substring(1);
    ...
    private void loadPrintedSources(Fragment fragment, String fragmentTEIID) {
        ...
        for (Element bibl : xp.evaluate(doc)) {
            ...
            if (whenAttribute == null) {
                ...
                printedSource.setDate(
                        DateUtils.convertDate(whenAttribute.getValue()));
            ...
            if (origin != null) {
                ...
                if (origDate != null) {
                    ...
                    if (when == null) {
                        ...
                        manuscript.setDate(
                                DateUtils.convertDate(when.getValue()));
    ...
    private void loadPhysDesc(Element msDesc, ManuscriptSource manuscript) {
        ...
        if (objectDesc.getAttributeValue("form").equals("leaf")) {
            ...
            throw new LdoDLoadException(
                    "não está definido o valor do atributo form="
                            + objectDesc.getAttributeValue("form") + " _VALOR_ "
                            + objectDesc.getContent());
        }
        ...
        manuscript.setColumns(
                Integer.parseInt(layoutElement.getAttributeValue("columns")));
    ...
    private Boolean isBreak(Element element) {
        ...
        if (breakWord == null || breakWord.equals("yes")) {
            ...
            throw new LdoDLoadException(
                    "valor inesperado para atribute break=" + breakWord);
    ...
    private SpaceText.SpaceUnit getUnitAttribute(Element element) {
        ...
        if (unitAttribute == null) {
            ...
            throw new LdoDLoadException("valor desconhecido para atributo unit="
                    + unitAttribute.getValue() + " _APLICADO AO VALOR_ "
                    + element.getText());
    ...
    private GapReason getGapReasonAttribute(Element element) {
        ...
        if (reasonAttribute != null) {
            ...
            default:
                throw new LdoDLoadException(
                        "valor desconhecido para atributo reason=" + reasonValue
                                + " dentro de gap");
            }
        }
        return reason;
    ...
    private String getFragmentXmlId(Element element) {
        ...
        if (xmlId == null) {
            throw new LdoDLoadException(
                    "falta xml:id de um fragmento" + " VALOR="
        ...
        if (getObjectDirectIdMap(xmlId) != null) {
            throw new LdoDLoadException(
                    "xml:id:" + xmlId + " já está declarado");
        }
        return xmlId;
    }
