...
public class DynamicDateProperty extends Property {
    ...
    public Collection<Double> extractVector(
        ...
        if (expertEditionInter.getDate() != null) {
            vector = buildVector(expertEditionInter.getDate().getYear(),
                    vector);
        }
        return vector;
    }
    ...
    private List<Double> buildVector(int date, List<Double> vector) {
        ...
        double j = getWeight();
        for (int i = start; i >= 0 && j > 0 && i < vector.size()
                && j >= vector.get(i); i--, j -= degree) {
            vector.set(i, j);
        }

        j = getWeight();
        for (int i = start; i < vector.size() && j > 0
                && j >= vector.get(i); i++, j -= degree) {
            vector.set(i, j);
        }
        return vector;
    ...
    protected Collection<Double> getDefaultVector() {
        return new ArrayList<Double>(
                Collections.nCopies(getNumberOfYears(), 0.));
    }
