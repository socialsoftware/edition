...
const importModule = async (module, file) => {
  const {
    mount,
    unMount
  } = await import(`../pages/${module}/${file}.js`);
  return {
...
const langSubscribe = () => unsubs.language = store.subscribe(updateLanguage());

const AboutRouter = (routes, language) => createElement("div", {
  id: "ldod-about-router-container"
}, createElement("div", {
  class: "container"
}, createElement("div", {
  class: "col-md-8 col-md-offset-2 ldod-about"
}, createElement("ldod-router", {
  base: "/about/",
  id: "about",
  routes: routes,
  language: language,
  class: "language"
}))), createElement("home-info", {
  language: language,
  class: "language"
}));
...
export const mount = (language, ref) => {
  ...
  const link = createElement("link", {
    id: "about",
    rel: "stylesheet",
    href: new URL(import.meta.url.includes('/public/') ? './css/about.css' : '../../css/about.css', import.meta.url).href
  });
  ...
  document.body.querySelector(ref).appendChild(AboutRouter(routes, language));
};
...
function updateLanguage() {
  return async (newV, oldV) => {
    if (oldV.language && newV.language !== oldV.language) {
      document.querySelector('div#ldod-about-router-container').querySelectorAll('.language').forEach(node => {
