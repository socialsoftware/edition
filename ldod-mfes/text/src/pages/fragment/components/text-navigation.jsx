...
import TextNavigationTable from './text-navigation-table';
...
const getExpertTableRow = (node, expert) => {
	return [
		...
		<input
			...
			type="checkbox"
			onChange={() => node.handleInterCheckboxChange(expert.externalId, expert.urlId)}
			checked={node.inters.has(expert.externalId)}
		/>,
		<a is="nav-to" to={textReferences.fragmentInter(expert.prevXmlId, expert.prevUrlId)}>
			<span class="icon icon-chevron-left"></span>
		</a>,
		<a is="nav-to" to={textReferences.fragmentInter(expert.xmlId, expert.urlId)}>
			{expert.number || '0'}
		</a>,
		<a is="nav-to" to={textReferences.fragmentInter(expert.nextXmlId, expert.nextUrlId)}>
...
export default ({ node }) => {
	return (
		<div id="text-navigation">
			...
			<br />
			<div id="text-expertEditionNavContainer" style={{ display: 'flex', justifyContent: 'center' }}>
				...
				<ldod-tooltip
					data-ref="div#text-expertEditionNavContainer span#text-navigation-title-tooltip"
				...
				<div class="text-center">
					{editorialInter[0] && (
						<TextNavigationTable
							...
							data={editorialInter.map(inter => getExpertTableRow(node, inter))}
						/>
					)}
				</div>
			))}
