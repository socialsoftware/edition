...
import(workerData).then((data) => {
  ...
  const checkApiKey = (key, type = 'function', required = true) => {
    if (required && (!contains(key) || typeof api[key] !== type))
      return parentPort.postMessage(
        `The MFE API must supply a ${key} as a ${type}`
      );
  ...
  const checkReferences = () => {
    ...
    if (
      Object.values(api.references).some(
        (reference) => typeof reference !== 'function'
      )
    )
      parentPort.postMessage(
        `The references should be encapsulated on a function`
      );
  };
