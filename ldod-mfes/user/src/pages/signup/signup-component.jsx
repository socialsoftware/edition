import { ALPHABETIC_REGEX, ALPHANUMERIC_REGEX, EMAIL_REGEX, PASSWORD_REGEX } from '@src/resources/constants.js';
...
export class SignUp extends HTMLElement {
	...
	getStateKeys() {
		return ['username', 'firstname', 'lastname', 'password', 'email', 'socialMedia', 'socialId', 'conduct'];
	}
	...
	async updateLanguage() {
		...
		['[data-key]'].forEach(selector =>
			this.querySelectorAll(selector).forEach(ele => (ele.innerHTML = this.getConstants(ele.dataset.key)))
		);
	...
	clearStyleInputs() {
		this.querySelectorAll('.form-control').forEach(div => div.classList.remove('valid'));
	}
	...
	hidePassword = ({ target }) => {
		target.parentElement.querySelector('input[name=password]').type = 'password';
	};
}
