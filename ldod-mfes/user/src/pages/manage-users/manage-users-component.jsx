...
export class ManageUsers extends HTMLElement {
	...
	updateUsersListTitleOnSearch = ({ detail }) => {
		if (detail.id === 'user-usersListTable') {
			const size = this.querySelectorAll('table#user-usersListTable tr[searched]').length;
	...
	handleChangeLanguage() {
		this.querySelectorAll('[data-key').forEach(ele => {
			ele.hasAttribute('title') && ele.setAttribute('title', this.getConstants(ele.dataset.key));
...
!customElements.get('manage-users') && customElements.define('manage-users', ManageUsers);
