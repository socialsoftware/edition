import { selectedInters } from '../event-module';
...
const loadTooltip = () => (import.meta.env.DEV ? import('@shared/tooltip.dev.js') : import('@shared/tooltip.js'));
...
export class VirtualNavigation extends HTMLElement {
	...
	fetchData = async () => {
		await getVirtualFragmentNavInters(this.fragment, {
			...
			currentInterId: this.intersChecked.length === 1 ? this.intersChecked[0] : null,
			urlId: this.urlId,
		})
	...
	handleChangedLanguage = () => {
		...
		this.shadowRoot
			...
			.forEach(node => node.setAttribute('content', this.getConstants(node.dataset.virtualTooltipKey)));
	};
...
!customElements.get('virtual-navigation') && customElements.define('virtual-navigation', VirtualNavigation);
