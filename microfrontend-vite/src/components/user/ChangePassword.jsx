...
export default () => {
  ...
  const checkPasswords = () => {
    ...
    if (changePasswordData.newPassword.length < PASSWORD_MIN_LENGTH) {
      setNewPasswordErrors((old) => [...old, messages[language]['Size']]);
      result = false;
    }

    if (changePasswordData.retypedPassword.length < PASSWORD_MIN_LENGTH) {
      setRetypedPasswordErrors((old) => [...old, messages[language]['Size']]);
      result = false;
    }
    ...
    if (changePasswordData.newPassword !== changePasswordData.retypedPassword) {
      setNewPasswordErrors((old) => [
        ...old,
        messages[language]['Different_changePasswordForm'],
      ]);
    ...
    if (checkPasswords()) {
      ...
      if (data === 'error')
        setCurrentPasswordErrors(
          messages[language]['DoNotMatch_changePasswordForm']
        );
  ...
  return (
    <div className="container text-center">
      <div className="row">
        <h3>{messages[language]['user_password']}</h3>
      </div>
      <div className="row">
        <br />
        <br />
        <form className="form-horizontal" onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="col-sm-4 control-label">
              {messages[language]['user_password_current']}
            </label>
          ...
          <div className="form-group">
            <label className="col-sm-4 control-label">
              {messages[language]['user_password_new']}
            </label>
            <div className="col-sm-4">
              <input
                className="form-control"
          ...
          <div className="form-group">
            <label className="col-sm-4 control-label">
              {messages[language]['user_password_retype']}
            </label>
            <div className="col-sm-4">
              <input
                className="form-control"
          ...
          <div className="form-group">
            <div className="col-sm-12">
              <button type="submit" className="btn btn-primary">
                {messages[language]['general_update']}
              </button>
            </div>
          </div>
        </form>
