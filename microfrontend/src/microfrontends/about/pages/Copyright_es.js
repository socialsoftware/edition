import React from 'react'



const Copyright_es = () => {
    return(
        <div>
<p style={{margin:"10px 0"}}>
	&copy; Manuel Portela y Antonio Rito Silva para los contenidos e
	infraestructura del <em>Archivo LdoD</em>.
</p>
<p style={{margin:"10px 0"}}>
	&copy; Herederos de Jacinto do Prado Coelho para la edici&oacute;n 1982
	del <em>Livro do Desassossego</em>.
</p>
<p style={{margin:"10px 0"}}>
	&copy; Teresa Cunha Sobral para la edici&oacute;n 2008 del <em>Livro
		do Desassossego</em>.
</p>
<p style={{margin:"10px 0"}}>
	&copy; Richard Zenith para la edici&oacute;n 2012 del <em>Livro do
		Desassossego</em>.
</p>
<p style={{margin:"10px 0"}}>
	&copy; Jer&oacute;nimo Pizarro para la edici&oacute;n 2010 del <em>Livro
		do Desassossego</em>.
</p>
<p style={{marginTop:"20px"}}>
	Todos los elementos en el <em>Archivo LdoD</em> pueden ser compartidos
	de acuerdo con una licencia Creative Commons &ldquo;<a
		href="https://creativecommons.org/licenses/by-nc/4.0/">Attribution-NonCommercial
		4.0 International</a>&rdquo;(CC BY-NC 4.0). Esta licencia no se aplica a
	las cuatro ediciones de los especialistas (Prado Coelho-1982; Sobral
	Cunha-2008; Zenith-2012; y Pizarro-2010), que <strong>s&oacute;lo
		se pueden utilizar dentro de la infraestructura del <em>Archivo
			LdoD</em>.
	</strong> La redistribuci&oacute;n o republicaci&oacute;n en otros
	t&eacute;rminos, en cualquier medio, requiere el consentimiento expreso
	y por escrito de los editores o sus representantes legales. <a
		href="https://github.com/socialsoftware/edition">El
	c&oacute;digo del <em>Archivo LdoD</em></a> puede ser compartido de acuerdo
	con una licencia <a href="https://en.wikipedia.org/wiki/BSD_licenses">"FreeBSD"</a>.

</p>

        </div>
        
    )
}

export default Copyright_es