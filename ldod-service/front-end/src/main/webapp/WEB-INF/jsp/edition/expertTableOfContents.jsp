...
<%--	${edition.editor} (${edition.getIntersSet().size()})--%>
	${edition.editor} (${edition.getExpertEditionInters().size()})
</h3>
...
<%--		<c:forEach var="interp" items='${edition.sortedInterps}'>--%>
			<c:forEach var="interp" items='${edition.getExpertEditionInters()}'>
			...
			<c:if
				...
				<tr>
					...
					<td><a
						href="${contextPath}/fragments/fragment/${interp.getFragmentXmlId()}/inter/${interp.getUrlId()}">${interp.title}</a></td>
					<td class="text-center"><a 
						href="${contextPath}/reading/fragment/${interp.getFragmentXmlId()}/inter/${interp.getUrlId()}/start"><span
