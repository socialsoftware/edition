    ...
    <table id="tableCategory" data-pagination="false"
        ...
        <c:forEach var="inter" items='${category.getSortedInters()}'>
            <tr>
                ...
                <td><a
                        href="${contextPath}/edition/acronym/${inter.getEdition().getAcronym()}">${inter.getEdition().getTitle()}</a>
                </td>
                <td><c:forEach var="user"
                               items="${inter.getContributorSet(category)}">
                    <a href="${contextPath}/edition/user/${user}">${userProvidesInterface.getFirtsName(user)} ${userProvidesInterface.getLastName(user)}
                        (${user})</a>
                    <br>
                </c:forEach></td>
