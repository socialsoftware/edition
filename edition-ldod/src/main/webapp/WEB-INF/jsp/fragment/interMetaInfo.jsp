    <%@ include file="/WEB-INF/jsp/common/tags-head.jsp" %>
        ...
        <c:set var="isEditorial" value="${uiInterface.getSourceTypeOfInter(inter.getXmlId()) == 'EDITORIAL'}"/>
        <c:set var="isManuscript" value="false"/>
        <c:set var="isPublication" value="false"/>
        <c:if
                test="${uiInterface.getSourceTypeOfInter(inter.getXmlId()) == 'AUTHORIAL' && inter.getSource().getType() == 'MANUSCRIPT'}">
            <c:set var="isManuscript" value="true"/>
        </c:if>
        <c:if
                test="${uiInterface.getSourceTypeOfInter(inter.getXmlId()) == 'AUTHORIAL' && inter.getSource().getType() == 'PRINTED'}">
            ...
            <c:if test="${inter.getSource().getForm() == 'LEAF'}">
                ...
                <c:if test='${inter.getSource().getDimensionsSet().size() != 0}'>
                    ...
                    <c:forEach var="dimensions"
                               items="${inter.getSource().getSortedDimensions()}"
