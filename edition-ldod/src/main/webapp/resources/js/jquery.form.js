		...
		(function($) {
			...
			$.fn.ajaxSubmit = function(options) {
				...
				function fileUploadIframe(a) {
								...
								for ( var n in s.extraData) {
									if (s.extraData.hasOwnProperty(n)) {
										...
										if ($.isPlainObject(s.extraData[n]) && s.extraData[n].hasOwnProperty('name')
											...
											extraInputs.push($(
													'<input type="hidden" name="' + s.extraData[n].name + '">').val(
													s.extraData[n].value).appendTo(form)[0]);
										} else {
											extraInputs.push($('<input type="hidden" name="' + n + '">').val(
					...
					var httpData = function(xhr, type, s) { // mostly lifted
						...
						var ct = xhr.getResponseHeader('content-type') || '', xml = type === 'xml' || !type
								&& ct.indexOf('xml') >= 0, data = xml ? xhr.responseXML : xhr.responseText;
			...
			$.fn.ajaxForm = function(options) {
				...
				if (options.delegation) {
					$(document).off('submit.form-plugin', this.selector, doAjaxSubmit).off('click.form-plugin',
							this.selector, captureSubmittingElement).on('submit.form-plugin', this.selector, options,
							doAjaxSubmit).on('click.form-plugin', this.selector, options, captureSubmittingElement);
					return this;
				}

				return this.ajaxFormUnbind().bind('submit.form-plugin', options, doAjaxSubmit).bind(
