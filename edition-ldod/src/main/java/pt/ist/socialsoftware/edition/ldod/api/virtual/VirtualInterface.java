package pt.ist.socialsoftware.edition.ldod.api.virtual;
...
public class VirtualInterface {
    private static final Logger logger = LoggerFactory.getLogger(VirtualInterface.class);
    ...
    public void notifyEvent(Event event) {
        if (event.getType().equals(Event.EventType.FRAGMENT_REMOVE)) {
            ...
            virtualModule.getPlayerSet().stream().filter(player -> player.getUser().equals(username)).forEach(player -> player.remove());
    ...
    public Set<Tag> getTagsForVei(String xmlID) {
        return VirtualModule.getInstance().getVirtualEditionInterByXmlId(xmlID).getTagSet();
    }
    ...
    public String getAcronymWithVeiId(String xmlId) {
        return VirtualModule.getInstance().getVirtualEditionsSet().stream().filter(virtualEdition -> virtualEdition.getFragInterByXmlId(xmlId) != null)
    ...
    public VirtualEdition getVirtualEdition(String acronym) {
        return VirtualModule.getInstance().getVirtualEdition(acronym);
    }
}
