...
public class ClassificationModule extends ClassificationModule_Base {
    ...
    public Set<ClassificationGame> getActiveGames4User(String username) {
        ...
        Set<VirtualEditionDto> virtualEditions4User = gameRequiresInterface.getVirtualEditionUserIsParticipant(username);
    ...
    public static void manageDailyClassificationGames(DateTime initialDate) {
        ...
        VirtualEditionDto virtualEdition = gameRequiresInterface.getVirtualEdition(acronym);

        // generate daily games
        for (int i = 0; i < 96; i++) {
            int index = (int) Math.floor(Math.random() * gameRequiresInterface.getVirtualEditionInterDtoSet(acronym).size());
            VirtualEditionInterDto inter = gameRequiresInterface.getVirtualEditionInterDtoSet(acronym).stream()
           ...
           createClassificationGame(virtualEdition,"Jogo " + formatedDate, date, inter, User.USER_ARS);
        }
