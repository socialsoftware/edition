...
//        }
    ...
    private Set<TwitterCitation> getTotalTwitterCitationsByInterAndCriteria(VirtualEditionInter inter,
        ...
        for (CitationDto tc : getFragment(inter).getCitationSet()) {
            if (tc instanceof TwitterCitationDto & validateCriteria(tc, criteria)) {
    ...
    private boolean validateCriteria(CitationDto tc, Set<SocialMediaCriteria> criteria) {
        ...
        for (SocialMediaCriteria criterion : criteria) {
            if (criterion instanceof MediaSource) {
                if (tc instanceof TwitterCitationDto && !((MediaSource) criterion).getName().equals("Twitter")) {
                    isValid = false;
                }
                ...
                if (tc instanceof TwitterCitationDto && !((GeographicLocation) criterion).containsEveryCountry()
                        && !((GeographicLocation) criterion).containsCountry(((TwitterCitationDto) tc).getCountry())) {
                    isValid = false;
                }
            }
        }
...
//        return TextModule.getInstance().getFragmentByXmlId(virtualEditionInter.getFragmentXmlId());
        return textProvidesInterface.getFragmentByXmlId(virtualEditionInter.getFragmentXmlId());
    }
...
//        return TextModule.getInstance().getScholarInterByXmlId(virtualEditionInter.getLastUsed().getXmlId());
        return textProvidesInterface.getScholarInter(virtualEditionInter.getLastUsed().getXmlId());
    }
